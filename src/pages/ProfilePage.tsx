...
export function ProfilePage({}: ProfilePageProps) {
  ...
  const handleSave = () => {
    setProfileData({
      ...
      technicalSkills: editForm.technicalSkills.split(',').map(s => s.trim()).filter(Boolean),
      softSkills: editForm.softSkills.split(',').map(s => s.trim()).filter(Boolean),
      projectTypes: editForm.projectTypes.split(',').map(s => s.trim()).filter(Boolean)
    });
    setIsEditing(false);
  };
             ...
             <div className="absolute -bottom-16 left-8 flex items-end">
                <div className="relative">
                  <img
                    ...
                    className="w-32 h-32 rounded-full border-4 border-white dark:border-[#121A2B] object-cover bg-white"
                  />
          ...
          <div className="pt-20 px-8 pb-8">
            <div className="flex justify-between items-start mb-6">
              <div className="flex-1">
                {isEditing ? (
                  <div className="mb-4">
                    <label className="block text-sm font-medium text-[#64748B] dark:text-[#94A3B8] mb-1">Full Name</label>
                    <input 
                      type="text" 
                ...
                <p className="text-violet-500 font-medium">@{profileData.username}</p>
              </div>
              ...
              <div>
                {isEditing ? (
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <input 
                        ...
                        onChange={(e) => setEditForm({...editForm, collegeYear: e.target.value})}
                        className="w-full px-4 py-2 rounded-xl bg-[#F8FAFC] dark:bg-[#0B1020] border border-black/10 dark:border-white/10 text-[#0F172A] dark:text-[#F8FAFC] focus:outline-none focus:border-violet-500 transition-colors"
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3 text-[#64748B] dark:text-[#94A3B8]">
                    ...
                    <span>{profileData.college} • {profileData.collegeYear}</span>
                  </div>
                )}
              </div>
              ...
              <div>
                {isEditing ? (
                  ...
                  <div className="flex items-center gap-3 text-[#64748B] dark:text-[#94A3B8]">
                    ...
                    <span>{profileData.preferredRole} • {profileData.availability}</span>
                  </div>
                )}
              </div>
              ...
              {isEditing ? (
                <div>
                  <textarea 
                    ...
                    onChange={(e) => setEditForm({...editForm, projectTypes: e.target.value})}
                  ...
                  />
                  <p className="text-xs text-[#64748B] dark:text-[#94A3B8]">Separate project types with commas</p>
                </div>
              ) : (
              ...
              {isEditing ? (
                <div>
                  <textarea 
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-[#F8FAFC] dark:bg-[#0B1020] border border-black/10 dark:border-white/10 text-[#0F172A] dark:text-[#F8FAFC] focus:outline-none focus:border-violet-500 transition-colors resize-none mb-2"
                  />
                  <p className="text-xs text-[#64748B] dark:text-[#94A3B8]">Separate skills with commas</p>
                </div>
              ) : (
                <div className="flex flex-wrap gap-2">
